...
router.post('/register', (req, res) => {
  try {
    ...
    const defaultAvatar = `https://api.dicebear.com/7.x/identicon/svg?seed=${encodeURIComponent(cleanUsername)}`;
    ...
    const newUser = getOne('SELECT id, name, username, email, avatar, role, created_at FROM users WHERE id = ?', [userId]);
...
router.post('/login', (req, res) => {
  try {
    ...
    const user = getOne(
      'SELECT id, name, username, email, password_hash, avatar, role, created_at FROM users WHERE email = ? OR username = ?',
      [identifier, identifier]
    );

    if (!user) {
      return res.status(401).json({ success: false, message: 'Invalid credentials. Please verify your email/username and password.' });
    }
